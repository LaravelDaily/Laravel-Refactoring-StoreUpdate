                        ...
                        <form action="{{ route('addresses.update', $address) }}" method="POST" novalidate>
                            ...
                            <div class="mt-4">
                                ...
                                <select name="country" id="country" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block mt-1 w-full">
                                    @foreach($countries as $country)
                                        <option value="{{ $country->id }}" {{ (int)old('country') === $country->id ? 'selected' : ($address->country_id === $country->id ? 'selected' : '') }}>
                                            {{ $country->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mt-4">
                                ...
                                <x-input id="number" class="block mt-1 w-full"
                                    type="text"
                                    name="number"
                                    :value="old('number') ?? $address->house_number"
                                    required />
                            </div>
                            ...
                            <div class="mt-4">
                                ...
                                <x-label for="is_billing" :value="__('Is billing address')" class="inline-block ml-1"></x-label>
                            </div>
